...
def gen_rand_2D_data(start, end, num):
    ...
    xt_train = np.column_stack((nums1, nums2))
    return convert_to_tensor(xt_train, dtype=float32)
...
def set_2D_boundaries(x_bc, y_bc, vec_sec_dim):
    ax_bound = x_bc[0]  # lower or upper bound
    bx_bound = x_bc[1]  # other bound
    ay_bound = y_bc[0]  # value at first bound
    by_bound = y_bc[1]  # value at second bound

    # extend the boundary positon tensors to match the length of the vec_sec
    ax_extended = np.resize(ax_bound, (vec_sec_dim.shape[0], 1))
    bx_extended = np.resize(bx_bound, (vec_sec_dim.shape[0], 1))

    # join the arrays of the boundary positions with the vec_sec
    ax = tf.concat([conv_data(ax_extended), vec_sec_dim], axis=1)
    bx = tf.concat([conv_data(bx_extended), vec_sec_dim], axis=1)

    # join the upper and lower bound to reform the position vector
    x = tf.concat([ax, bx], axis=0)

    # extend the boundary value tensors to match the length of the vec_sec
    ay_extended = tf.constant(ay_bound, shape=(vec_sec_dim.shape[0], 1))
    by_extended = tf.constant(by_bound, shape=(vec_sec_dim.shape[0], 1))

    # join the values for the upper and lower positons
    y = tf.concat([ay_extended, by_extended], axis=0)
